...
#endif
#ifdef MAGNETICSEED 
#ifdef MAGNETICSEEDNOSTARTS
	  flag = 0; // Do NOT Cool only seed (there is anoter flag to avoid generate starts)
#endif
#endif
...
#endif
#ifdef MAGNETICSEED
	             SphP[i].MagSeed = fabs(egyeff) * 5E-2;   // This is the definition of how much energy we will put in MF
		     egyeff-= SphP[i].MagSeed;                 //Here we also substract that to the feedback

	             SphP[i].MagSeed *= All.UnitMass_in_g / All.UnitEnergy_in_cgs * SphP[i].d.Density * a3inv *All.Time ;// * a3inv
#endif
